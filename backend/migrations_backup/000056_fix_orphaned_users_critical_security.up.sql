-- CRITICAL SECURITY FIX: Fix orphaned users and implement Row Level Security
-- 業務重要性：⭐⭐⭐⭐⭐
...
-- Step 4: 建立管理員 bypass 政策（僅供系統管理使用）
-- 管理員可以在特殊情況下存取所有公司資料

-- 為每個表格建立管理員政策
CREATE POLICY admin_bypass_warehouses ON warehouses 
    FOR ALL
    TO admin_role
    USING (true)
    WITH CHECK (true);

CREATE POLICY admin_bypass_product_categories ON product_categories 
    FOR ALL
    TO admin_role
    USING (true)
    WITH CHECK (true);
...
-- 註記：admin_role 需要在應用層設定，此處僅準備政策
