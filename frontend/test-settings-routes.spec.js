...
test.describe('NexusERP Settings Module Routes', () => {
  ...
  test('should navigate to settings company page correctly', async ({ page }) => {
    ...
    // 驗證頁面標題 - 使用更精確的選擇器
    await expect(page.locator('[data-testid="company-settings-page"] h2')).toContainText('公司資訊');
  ...
  test('should navigate to settings units page correctly', async ({ page }) => {
    ...
    // 驗證頁面標題 - 使用更精確的選擇器
    await expect(page.locator('[data-testid="units-settings-page"] h2')).toContainText('計量單位管理');
  ...
  test('should navigate to settings categories page correctly', async ({ page }) => {
    ...
    // 驗證頁面標題 - 使用更精確的選擇器
    await expect(page.locator('[data-testid="categories-settings-page"] h2')).toContainText('產品類別管理');
  ...
  test('should navigate to settings permissions page correctly', async ({ page }) => {
    ...
    // 驗證頁面標題 - 使用更精確的選擇器
    await expect(page.locator('[data-testid="permissions-settings-page"] h2')).toContainText('使用者權限管理');
  ...
  test('should display correct page titles and breadcrumbs', async ({ page }) => {
    ...
    for (const testData of pageTests) {
      await page.goto(testData.url);
      
      // 驗證頁面標題 - 使用更精確的選擇器
      const pageTitle = page.locator('[data-testid*="-settings-page"] h2').first();
      await expect(pageTitle).toContainText(testData.pageTitle);
      
      // 驗證麵包屑描述 - 使用更精確的選擇器
      const pageDescription = page.locator('[data-testid*="-settings-page"] p').first();
      await expect(pageDescription).toContainText(testData.breadcrumb);
    }
  });
