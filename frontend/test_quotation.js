...
(async () => {
  ...
  try {
    // 步驟1: 前往主頁
    console.log('📍 步驟1: 前往 http://127.0.0.1:8000');
    ...
    await page.goto('http://127.0.0.1:8000/quotations/create');
    await page.waitForTimeout(3000);
    ...
    const productNameInput = await page.locator('input[name="product_name"], input[placeholder*="產品"]').first();
    ...
    // 填寫數量
    const quantityInput = await page.locator('input[name="quantity"], input[placeholder*="數量"]').first();
    ...
    // 填寫單價
    const priceInput = await page.locator('input[name="price"], input[name="unit_price"], input[placeholder*="價格"], input[placeholder*="單價"]').first();
